...
static struct fuse_ring *fuse_uring_create(struct fuse_conn *fc)
{
	...
	max_payload_size = max_t(unsigned int, FUSE_MIN_READ_BUFFER, fc->max_write);
...
static int fuse_import_ubuf(int rw, void __user *buf, size_t len, struct iov_iter *i)
{
	if (len > MAX_RW_COUNT)
		len = MAX_RW_COUNT;
				     ...
				     struct fuse_ring_ent *ent)
{
	...
	err = fuse_import_ubuf(ITER_SOURCE, ent->payload, ring->max_payload_sz,
			  &iter);
	if (err)
		return err;
				   ...
				   struct fuse_ring_ent *ent)
{
	...
	err = fuse_import_ubuf(ITER_DEST, ent->payload, ring->max_payload_sz, &iter);
	if (err) {
